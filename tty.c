...
int ttywrite(int dev, char *buf, int nchar)
{
	...
	if (enqueue( &(tty->tbuf), buf[i])!=QMAX) {
   ...
   struct tty *tty = (struct tty *)(devtab[dev].dvdata);
    ...
    switch( lsr = lsr & UART_IIR_ID)
    {
      ...
      case UART_IIR_THRI:
        ...
        else if (queuecount( &tty->tbuf ) == FULLQUE)  { /* if tbuf queue is full (6 chars) */
            /* call sleep() to set processes in RUN state to BLOCKED */
            sleep();
            /* dequeue a char */
            ch =dequeue(&tty->tbuf);
            /* now the queue has room, so call wakeup() to set processes in BLOCKED state back to RUN */
            wakeup();
            sprintf(log, ">%c", ch);
            debug_log(log);
            outpt( baseport+UART_TX, ch ) ; /* ack tx dev */
